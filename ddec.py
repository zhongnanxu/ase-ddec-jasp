...
def check_installation():
    ...
    try:
        ...
        raise 'Please make sure your "DDEC_PATH" variable is defined'
    if (not isfile(ddec_path + '/chargemol_job.m') 
        and not isdir(ddec_path + 'chargemol_10_10_2012')):
        raise 'Please make sure the DDEC program is installed.'
    return True

def get_charges(self):
    """Get array of charges.""" 
    # If the atoms are not attached to a jasp object, resume normal behavior
    if not isinstance(self, Vasp):
        if 'charges' in self.arrays: 
            return self.arrays['charges'].copy() 
        else: 
            return np.zeros(len(self.atoms))

    # Make a quick check to make sure this module is installed correctly
    if not check_installation():
        raise 'Make sure the DDEC program is installed correctly'
    ...
    # If the job has been started but not complete or in the queue, we need to
...
'''
    ...
    p = Popen(['qsub', '-joe', '-N', self.calc.vaspdir + '-DDEC', resources],
...
def read_output(atoms):
    '''The main output we are interested in is reading the xyz file with all of the charges'''
...
def check_vasp_complete():
    ...
    if not ((calc.int_keys['nsw'] == 0 or calc.int_keys['nsw'] == None)
            and calc.bool_keys['lcharg'] == True
            and calc.bool_keys['laechg'] == True
            and calc.string_keys['prec'] == 'Accurate'):
